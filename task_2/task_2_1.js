const getDayInfo = (date) => {
    ...
    if (!regExp.test(date)) {
        ...
        if (+year < 100) {
            return `Дата ${date} не поддерживается! Введите значение года не меньше 100.`
        } else if (+month > 12 || +number > daysInMonths[+month - 1]) {
